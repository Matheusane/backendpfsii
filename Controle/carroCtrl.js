...
export default class CarroCtrl {
    ...
    gravar(requisicao, resposta) {
        ...
        if (requisicao.method === 'POST' && requisicao.is('application/json')) {
            ...
            const anoFabricacao = dados.anoFabricacao;
            const qtdEstoque = dados.qtdEstoque;
            const marca = dados.marca;

            
            if (nome && valor > 0 && anoFabricacao
                && qtdEstoque >= 0 && marca) {
                const carro = new Carro(0, nome, valor, 
                                        anoFabricacao, qtdEstoque, 
                                        marca
                );
                //resolver a promise
                carro.gravar().then(() => {
    ...
    atualizar(requisicao, resposta) {
        ...
        if ((requisicao.method === 'PUT' || requisicao.method === 'PATCH') && requisicao.is('application/json')) {
            ...
            const qtdEstoque = dados.qtdEstoque;
            const marca = dados.marca;
            if (codigo && nome && valor > 0 && anoFabricacao
                && qtdEstoque >= 0 && marca) {
                const carro = new Carro(codigo, nome, valor,
                     anoFabricacao, qtdEstoque, marca);
                //resolver a promise
